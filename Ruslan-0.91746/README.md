...
- **Двухфазное обучение** (замороженный энкодер → полная дообучка)
...
1. **Загрузка данных** — Копирование датасета из Google Drive в runtime Colab
2. **Стратифицированное разбиение** — `StratifiedKFold` (5 сплитов) с группировкой по IP платформы для предотвращения утечки данных
3. **Двухфазное обучение**:
   - **Фаза 1 (3 эпохи):** Энкодер заморожен, обучается только декодер
   - **Фаза 2 (57 эпох):** Полная дообучка всей модели
4. **Планировщик скорости обучения**:
   - **Warmup:** Линейный warmup на 6 эпох (start_factor=0.005)
   - **Cosine Annealing:** LR убывает с 7-й по 60-ю эпоху (eta_min=1e-6)
5. **Ранняя остановка:** Patience = 10 эпох по метрике Dice
