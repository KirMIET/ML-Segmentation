...
- **Для быстрого запуска**: используйте одну модель с `use_sam=False`
- **Для лучшего качества**: ансамбль из нескольких архитектур с SAM + TTA
- **Для экономии памяти**: уменьшите `batch_size` и отключите multi-scale
- **Для воспроизводимости**: установите `SEED = 42` и `torch.backends.cudnn.deterministic = True`
